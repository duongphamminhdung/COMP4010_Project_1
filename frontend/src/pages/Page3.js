...
import { buildComparison } from "../utils/penguinData";
...
function Page3({ onBack }) {
    ...
    const fetchComparison = async () => {
      try {
        ...
        const result = await buildComparison(
          groupA.species, groupA.island, groupA.gender,
          groupB.species, groupB.island, groupB.gender,
        );
        setComparison(result);
      } catch (fetchError) {
        setError("Failed to load penguin data. Please refresh the page.");
        setComparison(null);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="page3">
      {/* Header matching project palette */}
      <header className="page3-header" role="banner">
        <div className="page3-header-left">
          <img src={process.env.PUBLIC_URL + '/vinuni_logo.png'} alt="VinUni Logo" className="page3-logo" />
          <div className="page3-course-info">
            <div className="page3-course-name">Data Visualization</div>
            <div className="page3-course-code">COMP4010</div>
          </div>
        </div>
        <div className="page3-header-center">
          <h1 className="page3-header-title">Penguin Group Comparison</h1>
          <p className="page3-header-subtitle">Select two groups and compare side-by-side</p>
        </div>
        <div className="page3-header-right">
          <button
            className="page3-back-btn"
            onClick={onBack}
      ...
      <div className="page3-selectors">
        <GroupSelector
          ...
          penguinStrokeColor="rgba(20, 75, 110, 0.4)"
        ...
        <GroupSelector
          ...
          penguinStrokeColor="rgba(130, 50, 80, 0.4)"
          showPenguin={showCardPenguins}
